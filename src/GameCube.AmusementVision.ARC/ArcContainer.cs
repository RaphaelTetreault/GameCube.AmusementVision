...
namespace GameCube.AmusementVision.ARC
{
    public class ArcContainer
    {
        ...
        {
            EndianBinaryReader inputBinaryStream = new EndianBinaryReader(inputStream, Endianness.BigEndian);
            ...
            U8Node[] nodes, ref int nodeIdx)
        {
            ...
            AsciiCString name = new AsciiCString(); // VALIDATE
            inputBinaryStream.Read(ref name); // VALIDATE

            if (currentNode.Type == U8Node.TYPE_FILE)
		    {
                inputBinaryStream.BaseStream.Position = currentNode.DataOffset;
                byte[] data = inputBinaryStream.ReadBytes(currentNode.Size);
        ...
        {
            EndianBinaryWriter outputBinaryStream = new EndianBinaryWriter(outputStream, Endianness.BigEndian);
            ...
            int dataBeginOffset = PaddingUtility.Align(headerSize + nodesSize + stringTableSize, 0x20); // VALIDATE
            ...
            foreach (ArcFileSystemEntry entry in entries)
                outputBinaryStream.Write(new AsciiCString(entry.Name)); // VALIDATE
            ...
            foreach (ArcFileSystemEntry entry in entries)
            {
                ...
                {
                    outputBinaryStream.WriteAlignment(0x20); // VALIDATE
            ...
            ref int currentNodeId, int parentNodeId, ref int currentStringTableOffset, ref int currentDataOffset)
        {
            ...
            else if (currentEntry is ArcFileSystemFile)
            {
                ...
                currentDataOffset = PaddingUtility.Align(currentDataOffset + currentFile.Data.Length, 0x20);
            }
        }
